...
def Process_InputDump(fileProcess,euCount):
	...
	infile = fileProcess
        ...
        headerdl= '\n'+'\n'+'Tick'+', '+ 'SFN'+', '+ ' SF '+',' +  'MAC TC(Diff in us)' +', '+' Enocde(Diff in us)' +', '+' MAC_NTC(Diff in us)'+'\n'
        with open(outfilename,'a') as f2:
                f2.write(headerdl)
	resfile = open(infile, 'r') 
	diff1=diff2=diff3=s1=m1=m2=m3=m4=m5=m6=0
	SYSFRAMENUM=SUBFRAME=0
	mac_start=mac_stop=encode_start=encode_stop=macnct_start=macnct_stop=["NA"]
	if 1:
		for line in resfile:
			...
			#sfnsf = re.search('0x0000', line)
				...
				if (m1 & m6):
					...
					diff1=diff2=diff3=0
					SYSFRAMENUM=SUBFRAME=0
					#mac_start[0]=mac_stop[0]=encode_start[0]=encode_stop[0]=macnct_start[0]=macnct_stop[0]=["NA"]
					mac_start=mac_stop=encode_start=encode_stop=macnct_start=macnct_stop=["NA"]
        				f2.write(dataEL)
...
def Sum_All_ELs():
	...
	for startLine in outFile:
		...
		t1=Sum_Mac_TC/i
		A1 = float("{0:.0f}".format(t1))
		t2=Sum_Encode/i
		A2 = float("{0:.0f}".format(t2))
		t3=Sum_Mac_NTC/i
		A3 = float("{0:.0f}".format(t3))
		A4=A1+A2
		sum_line='\n'+ 'Avg Time in ['+str(i)+'] Ticks:'+', '+ 'Mac TC:'+','+  str(A1)+' us'+'\n' +', '+'Encode :'+','+ str(A2) +' us'+'\n'+', '+'Mac NTC :'+','+ str(A3)+' us' +'\n'+','+' MAC_TC+Encode EL:'+','+str(A4)+' us'+'\n'
	        f = open('Output__Mac_ELs.csv')
		text = f.read()
		f.close()
		f = open('Output__Mac_ELs.csv', 'w')
		f.write(sum_line)
...
def CpuInfo():
	...
	cpuList=["NA","NA","NA","NA"]
        for line in resfile:
                matchCpu0 = re.search('Time analysis for CPU#0', line)
                if matchCpu0:
			break

	for line in resfile:
        	mid0 = re.search('CPU IDLE', line)
		if mid0 :
			pattern=line.split() # split line into parts
			tempValue = re.sub("\D", "", pattern[6])
			check=1
       	                cpuList.insert(i,tempValue)
			i=i+1
	if check ==1:
		cpuheader= '\n'+'IDLE CPU-L Info >> '+','+ ' CPU#0 : '+','+ str(cpuList[0])+" %"+'\n'+','+ ' CPU#1 : '+','+str(cpuList[1])+"  %"+'\n'+','+' CPU#2 : '+','+str(cpuList[2])+" %"+'\n'+','+' CPU#3 : '+','+str(cpuList[3])+" %"+ '\n'
        	with open('Output__Mac_ELs.csv','a') as f1:
               		f1.write(cpuheader)
	        	f1.close()
	else:
		print "\n No Cpu Info present in res.tct"
	
def ParseCpu():
	LinesGap='\n'+'\n'
	cpuheader=LinesGap+','+', '+', '+ ' CPU Profiles for 4 Cores on CPU L ' + '\n'
	with open('Output__Mac_ELs.csv','a') as f1:
       		f1.write(cpuheader)
		f1.close()
        resfile = open('res.txt', 'r')
        for i,startL in  enumerate(resfile):
                matchCpu = re.search('Log end', startL)
...
if __name__ == '__main__':
	...
	Sum_All_ELs()
        print "****Post Processing Successfully Completed >> Output__Mac_ELs.csv "
	ParseCpu()
        #List All OutPut Files
        ListFiles()
